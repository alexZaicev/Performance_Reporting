...
class RGButtonsFrame(RGComponentBase):
    B_GENERATE = 'BG'
    B_CANCEL = 'BCN'
    ...
    def build(self):
        ...
        self.__widgets = {
            RGButtonsFrame.B_GENERATE: RGButton(window=frame, config=self.config, text=GENERATE.upper(),
                                                callback=lambda: Thread(target=self.__generate_report).start(),
                                                width=self.size[0] * 0.02, height=self.size[1] * 0.04,
                                                font_weight='bold', color=cons.BLUE,
                                                x=self.size[0] * 0.81, y=self.size[1] * 0.11),
            RGButtonsFrame.B_CANCEL: RGButton(window=frame, config=self.config, text=CANCEL.upper(),
                                              callback=self.__cancel_report,
    ...
    def __generate_report(self):
        if not self.window.is_blocked():
            ...
            if err_msg is not None:
                ...
                self.__conf_dialog = RGConfirmDialog(window=self.window, config=self.config, message=msg,
                                                     p_text=CONTINUE, p_callback=self.__run_report, n_text=ABORT)
    ...
    def __run_report(self):
        ...
        try:
            RGToolManager.execute(self.config, self.window, loader)
