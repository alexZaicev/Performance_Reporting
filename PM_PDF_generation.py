...
pdf.add_font('DejaVuSans', '', os.path.join(ROOT_DIR, 'resources', 'fonts', 'DejaVuSans.ttf'), uni=True)
pdf.add_font('DejaVuSans', 'B', os.path.join(ROOT_DIR, 'resources', 'fonts', 'DejaVuSans.ttf'), uni=True)
...
def main():
    ...
    for idx, line in df_measure.iterrows():
        ...
        measure_data_df.loc[:, "YearMonth"] = measure_data_df.loc[:, "Fiscal Year"].str.replace("-", "").str.cat(
            measure_data_df.loc[:, "Month"].str[1:3])
        measure_data_df.loc[:, "YearQuarter"] = measure_data_df.loc[:, "Fiscal Year"].str.replace("-", "").str.cat(
            measure_data_df.loc[:, "Quarter"].str[1:2])
...
def read_template(parent_dir):
    ...
    :return:
    """
    parent_dir = parent_dir + "/*/2019-20*template.xlsx"
    df_measure = pd.DataFrame()
    df_measure_data = pd.DataFrame()
    for file in glob.iglob(parent_dir):
        print("Reading file ", str(file))
        file = file.replace("\\", "/")
        dict_template = pd.read_excel(file, sheet_name=["CurrentYearData", "CurrentYearMeasures"], encoding='utf-8')
...
def create_barchart(m):
    ...
    if freq.upper() in ["M", "MONTHLY", "MONTH"]:
        ...
        print("Frequency '" + freq + "' of measure " + str(
            m.measure.measure_id) + " not recognised. By default we assume it is monthly.")
        x_frequency = "YearMonth"
        x_numerical = list(map(int, m.data[x_frequency].values))
        x_ticklabels = m.data["Month"].str[6:9]
        y_target = list(map(float, pd.to_numeric(m.data["Target"], errors="coerce")))
    ...
    # print(m.data[result_indexes])
    df_latest_results = m.data.loc[result_indexes, ["Result", x_frequency, "Performance"]]
    ...
    result_Not_BRAG_GREY = list(
        map(float, pd.to_numeric(df_latest_results[is_perf_Not_BRAG]["Result"], errors="coerce")))
    ...
    if len(x_numerical) == 1 & ~df_latest_results.empty:
        ...
        ax.set_xlim(int(df_latest_results[x_frequency].values[0]) - 1,
                    int(df_latest_results[x_frequency].values[0]) + 1)
...
def add_dot_image(pref_dot, x, y):
    # TODO handle the new DOT symbols (not in Test files but in actual files).
    #  watchout for the ~(dot in ["p", "q", "r", "s", "u"])
    # print(pref_dot)
    if pref_dot == "p":
...
def format_value(value, data_format):
    ...
    if 'nan' in str(value).lower():
        value = "N/A"
    elif math.isnan(pd.to_numeric(value, errors="coerce")):
...
def create_visual(m, x_origin, y_origin):
    ...
    commentary_step2 = unicodedata.normalize('NFKC', commentary_step1).encode('cp1252', 'replace').decode("utf-8",
                                                                                                          'ignore')
